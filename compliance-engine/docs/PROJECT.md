...
**The Zero-Trust Mandate:** The Central API (Control Plane) never ingests, processes, or stores raw Personally Identifiable Information (PII). All cryptographic mutation, relational graph discovery, and data redaction occur locally via the worker. The worker maintains Egress-Only network access to poll for instructions and dispatch Certificates of Erasure (CoEs).
...
The worker utilizes native C++ bindings via the **Bun Web Crypto API** (`globalThis.crypto`) for maximum concurrency and compliance with strict audit standards.
  ...
  * **Envelope Encryption:** \* **KEK (Key Encrypting Key):** A 256-bit AES master key residing strictly in the worker's RAM.
...
Vaulting is Stage 1. The worker operates entirely autonomously to complete the compliance lifecycle based on the time limits explicitly declared in the YAML.

### The Notice Handshake (Stage 2)

  * **When:** Defined by `compliance_policy.notice_window_hours` (e.g., 48 hours before retention expiry).
...
To ensure the engine can be safely deployed and monitored by enterprise DevOps teams:
  ...
  * **Containerization:** The worker is packaged as a Distroless Docker image (based on `oven/bun:alpine`), maintaining a footprint of \< 150MB with zero shell access.
  ...
  * **Shadow Mode:** The worker supports a reversible dry-run configuration. It executes the full $O(V+E)$ graph discovery, locking, and cryptographic overhead, but executes a forced `tx.rollback()` at the end to allow for exact performance benchmarking on production infrastructure without mutating data.
